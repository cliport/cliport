...
class CLIPORT:
    ...
    def run(self, obs):
        ...
        self.act = act
        self.pick, self.place  = act['pose0'], act['pose1']
...
def write_pickle_file(data):
    ...
    with open(f'results/{filename}', 'wb') as fd:
        pickle.dump(data, fd)
    ...
    try:
        ...
        while not rospy.is_shutdown():
            if streamer.rgb is not None and streamer.depth is not None and cliport.lang is not None:
                ...
                # Get 3d centroid for pick/place
                ...
                data_dic = {'pick_xyz': pick_xyz, 'pick_rotation': pick_rotation, 'place_xyz': place_xyz, 'place_rotation': place_rotation}
