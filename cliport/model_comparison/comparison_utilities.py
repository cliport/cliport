...
class DataHandler:
    ...
    def rot_on_model(self, episode:tuple, mode: str, location: str) -> list[np.ndarray]:
        ...
        """
        if mode == "train":
            batch = self.training_dataset.process_sample(episode)
        elif mode == "val":
            batch = self.validation_dataset.process_sample(episode)
        else:
            return None, None
...
class DataProcessor:
    ...
    @staticmethod
    def find_peak(prediction_data: list) -> float:
    ...
    def calculate_travel_error(
        ...
        actual_travel = self.calculate_pythagorean_distance(place_actual, pick_actual)[3]
        predicted_travel = self.calculate_pythagorean_distance(place_prediction, pick_prediction)[3]
    ...
    @staticmethod
    def convert_dict_to_csv(dict_to_convert: dict) -> str:
        csv_text = ""
        task_names = list(dict_to_convert.keys())
        csv_text += "error,"
        for task_name in task_names:
            # first line containing error legends
            csv_text += f"{task_name},"
        csv_text += '\n'
        row_entries = {}
        for task_name in task_names:
            # extracting data for rows of csv
            i = 0
            while i < len(dict_to_convert[task_name]):
                error_title = dict_to_convert[task_name][i+1]
                error = dict_to_convert[task_name][i]
        ...
        for entry in list(row_entries.keys()):
            csv_text += f'{entry},'
            for item in row_entries[entry]:
                csv_text += f'{item},'
            csv_text += '\n'
        return csv_text
    
    def collapse_results_to_meta_results(self, all_data_dict: dict) -> dict:
        # TODO: finish this
        result_dict = {}
        all_lang_goals = {}
        ...
        for model_name in model_names:
            ...
            for lang_goal in lang_goals:
                if lang_goal not in all_lang_goals:
                    all_lang_goals[lang_goal] = all_data_dict[model_name]['lang_goals'][lang_goal]
                else:
                    all_lang_goals[lang_goal] += all_data_dict[model_name]['lang_goals'][lang_goal]
        ...
        for model_name in model_names:
            ...
            present_example_lang_goals = self.find_task_lang_goals(all_data_dict[model_name])
            for lang_goal in all_lang_goals:
                skip_flag = False
                if lang_goal in present_example_lang_goals:
                    current_goal_data = self.find_same_tasks(all_data_dict[model_name], lang_goal)
                else:
                    skip_flag = True
                    current_goal_data = self.create_empty_task()
                    
                if not skip_flag:
                    averaged_goal_data, average_order = self.calculate_average_of_errors(current_goal_data)
                    
                    order_stash.append(average_order)
                    if model_name not in result_dict:
                        result_dict[model_name] = {lang_goal: averaged_goal_data}
                    else:
                        result_dict[model_name][lang_goal] = averaged_goal_data
    ...
    def create_empty_task() -> list:
        ...
        return([{
            ...
            'place_z_error': 0,
            'total_place_error': 0,
            'pick_rotation_error': 0,
            'place_rotation_error': 0,
            'travel_error': 0,
            'actual_travel': 0,
            'predicted_travel': 0,
        }])
