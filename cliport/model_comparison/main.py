...
def read_model(data_extractor: DataHandler, index: int):
    print(f"\nReading model: {MODELS[index]} ({index+1}/{len(MODELS)})\n")
    # read values from directories (data storage inside extractor)
    training_size = data_extractor.get_set_limit(MODELS[index], "train")
    if not USE_AMALGAM:
        validation_size = data_extractor.get_set_limit(MODELS[index], "val")
        data_extractor.read_dataset(MODELS[index], "val")
    else:
        validation_size = data_extractor.get_set_limit(AMALGAM_TITLE, "val")
        data_extractor.read_dataset(AMALGAM_TITLE, "val")
        
    lang_goals = data_extractor.get_lang_goals(MODELS[index])
    
    data_extractor.read_dataset(MODELS[index], "train")
    data_extractor.find_latest_best_checkpoint_version(
        MODELS[index], MODEL_EXTENDERS[index]
    )
    data_extractor.augment_cfg(MODELS[index], MODEL_EXTENDERS[index])
    data_extractor.load_model(MODELS[index], MODEL_EXTENDERS[index])
